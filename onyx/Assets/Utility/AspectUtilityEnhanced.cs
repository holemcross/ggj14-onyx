...
public class AspectUtilityEnhanced : MonoBehaviour {
    ...
    static Camera cam;
	static tk2dCamera tk2dCam;
    static Camera backgroundCam;
 
    void Awake () {
		_landscapeModeOnly = landscapeModeOnly;
        cam = camera;
		tk2dCam = gameObject.GetComponent<tk2dCamera>();
		if(tk2dCam) cam = tk2dCam.camera;
    ...
    public static void SetCamera () {
        ...
        if (currentAspectRatio > wantedAspectRatio) {
			...
			if(tk2dCam) tk2dCam.CameraSettings.rect = new Rect(inset/2, 0.0f, 1.0f-inset, 1.0f);
        }
        // Letterbox
        else {
            float inset = 1.0f - currentAspectRatio/wantedAspectRatio;
            cam.rect = new Rect(0.0f, inset/2, 1.0f, 1.0f-inset);
			if(tk2dCam) tk2dCam.CameraSettings.rect = new Rect(0.0f, inset/2, 1.0f, 1.0f-inset);
        }
        if (!backgroundCam) {
